...
parser.add_argument("--bgcolor", help="background color",
                    nargs='+', type=int, default=(255, 255, 255, 255))
...
parser.add_argument("--fontcolor", help="font color",
                    nargs='+', type=int, default=(0, 0, 0, 255))
parser.add_argument("--fontfamily", help="fontfamily",
                    type=str, default="fontfamily/wqy-microhei.ttc")
...
# background image
print("background image: " if args.bgimage else "background color:",
      args.bgimage or args.bgcolor)
...
if args.border != None:
    ...
    draw.rectangle((150, 150, width-150, height-150),
                   outline=args.border, width=args.borderwitdh)
...
if args.resize != None:
    ...
    if width > height:
        target = target.resize(
            (args.resize, round(height*(args.resize/width))))
    else:
        target = target.resize(
            (round(width*(args.resize/height)), args.resize))
